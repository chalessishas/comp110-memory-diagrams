"""Virtuosa S2 "Requiem Mass" — 20s MANUAL, ASPD+45. S1/S3 stubs.

Tests cover:
  - S3 config (name, sp_cost=60, initial_sp=32, duration=34s)
  - S2 config (name, sp_cost=30, initial_sp=14, duration=20s)
...
from data.characters.cello import (
    make_cello, _S1_TAG, _S2_TAG, _S2_ASPD_BONUS, _S2_BUFF_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION,
)
...
def test_cello_s2_config():
    op = make_cello(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Requiem Mass"
    assert sk.sp_cost == 30 and sk.initial_sp == 14
