...
from data.characters.skadi import (
    make_skadi, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_skadi_s2_config():
    s = make_skadi(slot="S2")
    sk = s.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Surge"
...
def test_s2_atk_buff_applied():
    ...
    assert s.effective_atk > base_atk, "S2 must increase ATK"
    assert any(b.source_tag == _S2_BUFF_TAG for b in s.buffs), "ATK buff must be present"
...
def test_s2_atk_amount():
    ...
    assert s.effective_atk == expected, f"expected ATK={expected}, got {s.effective_atk}"
...
def test_s2_buff_cleared_on_end():
    ...
    assert s.effective_atk == base_atk, "ATK must return to base after S2"
...
def test_s3_regression():
    ...
    assert s.skill.name == "Abyssal Resonance"
