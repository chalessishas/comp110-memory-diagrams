"""Hoshiguma S1 "Warpath" — 30s MANUAL, ATK+40%, DEF+80%.

Tests cover:
  - S1 config (name, sp_cost=40, initial_sp=20, duration=30s)
  - S1 ATK buff applied on trigger
  - S1 DEF buff applied on trigger
  - S1 both buffs cleared after skill ends
  - S2/S3 slot regression
"""
...
from data.characters.hsguma import (
    make_hsguma,
    _S1_TAG, _S1_ATK_RATIO, _S1_DEF_RATIO,
    _S1_ATK_BUFF_TAG, _S1_DEF_BUFF_TAG, _S1_DURATION,
    _S2_TAG, _S3_TAG, _S3_DURATION,
)
...
def test_hsguma_s1_config():
    op = make_hsguma(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Warpath"
...
def test_s3_slot_config():
    op = make_hsguma(slot="S3")
    sk = op.skill
    assert sk is not None and sk.slot == "S3"
    assert sk.sp_cost == 50 and sk.initial_sp == 30
