"""Flint S2 "Smelt and Strike" — ATK+70% for 11s, AUTO trigger.
...
from data.characters.flint import (
    make_flint, _S2_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_flint_s2_config():
    op = make_flint(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Smelt and Strike"
    assert sk.sp_cost == 24
    assert sk.initial_sp == 12
...
def test_s2_atk_amount():
    ...
    op.skill.sp = float(op.skill.sp_cost)
    w.tick()
    # Talent adds persistent ATK buff; check S2 buff value directly
...
def test_s2_buff_cleared_on_end():
    ...
    for _ in range(int(TICK_RATE * (_S2_DURATION + 1.0))):
        w.tick()
    # Talent buff may persist; only verify S2 buff is removed
    assert not any(b.source_tag == _S2_BUFF_TAG for b in op.buffs)


def test_s3_regression():
    op = make_flint(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "White-Hot Forge"
