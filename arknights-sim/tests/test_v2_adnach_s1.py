"""Adnachiel S1 "ATK Up α" — 20s MANUAL, ATK+50%.
...
Tests cover:
  ...
  - Talent ASPD+8 applied on deploy
"""
...
from data.characters.adnach import (
    make_adnach, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
    _TALENT_BUFF_TAG, _TALENT_ASPD_BONUS,
)
...
def test_adnach_s1_config():
    op = make_adnach(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "ATK Up α"
...
def test_talent_aspd_on_deploy():
    w = _world()
    op = make_adnach(slot="S1")
    ...
    assert op.effective_aspd == base_aspd + _TALENT_ASPD_BONUS
