"""Ambriel (glaze) — 4★ Sniper/Deadeye. S1: ATK+30%/30s MANUAL, S2: ATK+100%/35s MANUAL.

Tests cover:
  - S1 config (sp_cost=30, initial_sp=10, duration=30s)
  - S2 config (sp_cost=25, initial_sp=10, duration=35s)
...
from data.characters.glaze import (
    make_glaze,
...
def test_glaze_s1_config():
    ...
    assert sk.sp_cost == 30 and sk.initial_sp == 10
    assert sk.duration == _S1_DURATION and sk.behavior_tag == _S1_TAG
    assert sk.sp == 10.0


def test_glaze_s2_config():
    op = make_glaze(slot="S2")
    sk = op.skill
...
def test_s1_atk_buff_applied():
    ...
    assert any(b.source_tag == _S1_BUFF_TAG and b.value == _S1_ATK_RATIO
...
def test_s2_atk_buff_applied():
    ...
    assert any(b.source_tag == _S2_BUFF_TAG and b.value == _S2_ATK_RATIO
               for b in op.buffs if b.axis == BuffAxis.ATK)
