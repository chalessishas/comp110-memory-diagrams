...
from data.characters.udflow import make_udflow, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_udflow_s1_config():
    op = make_udflow(slot="S1")
    sk = op.skill
...
def test_udflow_s2_config():
    op = make_udflow(slot="S2")
    sk = op.skill
...
def test_udflow_s3_config():
    op = make_udflow(slot="S3")
    sk = op.skill
