"""Logos S2 "Theorem Derivation" — ATK+60% for 20s, AUTO trigger.
...
from data.characters.logos import (
    make_logos, _S2_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_s3_regression():
    op = make_logos(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Absolute Logos"
