...
from data.characters.toddi import make_toddi, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_toddi_s1_config():
    op = make_toddi(slot="S1")
    sk = op.skill
...
def test_toddi_s2_config():
    op = make_toddi(slot="S2")
    sk = op.skill
...
def test_toddi_s3_config():
    op = make_toddi(slot="S3")
    sk = op.skill
