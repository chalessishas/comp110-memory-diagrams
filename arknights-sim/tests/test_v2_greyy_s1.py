"""Greyy S1 "Tactical Chant β" — 25s MANUAL, ASPD+75.
...
Tests cover:
  ...
  - S2 slot regression (name="Electrostatic Discharge")
"""
...
from data.characters.greyy import (
    make_greyy, _S1_TAG, _S1_ASPD_BONUS, _S1_ASPD_BUFF_TAG, _S1_DURATION,
)
...
def test_greyy_s1_config():
    op = make_greyy(slot="S1")
    sk = op.skill
...
def test_s2_slot_config():
    op = make_greyy(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Electrostatic Discharge"
