"""Glacus S2 "Arctic Barrier" — ATK+40% for 20s, AUTO trigger (requires_target).
...
from data.characters.glacus import (
    make_glacus, _S2_ATK_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_glacus_s2_config():
    op = make_glacus(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Arctic Barrier"
...
def test_s3_regression():
    ...
    assert op.skill.name == "Arctic Tempest"
