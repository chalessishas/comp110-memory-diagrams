"""Aosta S1 "Swift Strike γ" — 35s MANUAL, ATK+45% ASPD+45.
...
from data.characters.aosta import (
    make_aosta, _S1_TAG, _S1_ATK_RATIO, _S1_ASPD_BONUS,
...
def test_aosta_s1_config():
    op = make_aosta(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_aosta(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Shadow Nails"
