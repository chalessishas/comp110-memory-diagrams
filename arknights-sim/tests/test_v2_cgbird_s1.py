...
from data.characters.cgbird import (
    make_cgbird, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_cgbird_s1_config():
    op = make_cgbird(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_cgbird(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Magic Shield"
