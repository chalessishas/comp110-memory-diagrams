"""Chen the Holungday S1 "Chixiao: Night Sprint" — 18s MANUAL, ATK+120%.

Tests cover:
  - S1 config (name, sp_cost=20, initial_sp=10, duration=18s, MANUAL)
...
from data.characters.chen3 import (
    make_chen3, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_chen3_s1_config():
    op = make_chen3(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Chixiao: Night Sprint"
...
def test_s2_regression():
    op = make_chen3(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Chixiao: Flash Drive"
