"""Carnelian (Billro) S1 "Sandstorm Guard" — 20s MANUAL, ATK+60% DEF+100%.

Tests cover:
  - S1 config (name, sp_cost=18, initial_sp=5, duration=20s, MANUAL)
...
from data.characters.billro import (
    make_billro, _S1_TAG, _S1_ATK_RATIO, _S1_DEF_RATIO,
...
def test_billro_s1_config():
    op = make_billro(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Sandstorm Guard"
...
def test_s2_regression():
    op = make_billro(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Sandstorm Blade"
