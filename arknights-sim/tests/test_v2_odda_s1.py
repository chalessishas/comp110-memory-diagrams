from data.characters.odda import make_odda, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_odda_s1_config():
    op = make_odda(slot="S1")
    sk = op.skill
...
def test_odda_s2_config():
    op = make_odda(slot="S2")
    sk = op.skill
...
def test_odda_s3_config():
    op = make_odda(slot="S3")
    sk = op.skill
