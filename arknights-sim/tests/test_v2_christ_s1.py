"""Miss.Christine S1 "Mealtime Manners" — 25s MANUAL, ATK+50%.

Tests cover:
  - S1 config (name, sp_cost=30, initial_sp=15, duration=25s, MANUAL)
...
from data.characters.christ import (
    make_christ, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_christ_s1_config():
    op = make_christ(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Mealtime Manners"
...
def test_s2_slot_config():
    op = make_christ(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Bacchanalia"
