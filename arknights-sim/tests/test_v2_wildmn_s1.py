"""Wild Mane S1 "Cavalry Thrust" — 30s ON_DEPLOY AUTO, ASPD+135.

Tests cover:
  - S1 config (sp_cost=0, initial_sp=0, duration=30s, ON_DEPLOY)
...
from data.characters.wildmn import (
    make_wildmn,
...
def test_wildmn_s1_config():
    op = make_wildmn(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Cavalry Thrust"
