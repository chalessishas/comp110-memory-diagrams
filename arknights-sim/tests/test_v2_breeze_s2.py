"""Breeze S2 "Revitalizing Gale" — ATK+30% for 20s, AUTO trigger.
...
from data.characters.breeze import (
    make_breeze, _S2_ATK_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_breeze_s2_config():
    op = make_breeze(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Revitalizing Gale"
...
def test_s3_regression():
    op = make_breeze(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Healing Storm"
