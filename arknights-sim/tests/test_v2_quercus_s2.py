"""Quercus S2 "Blessing of the Woods" — ATK+20% for 20s, AUTO trigger (requires_target).
...
from data.characters.quercus import (
    make_quercus, _S2_ATK_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_quercus_s2_config():
    op = make_quercus(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Blessing of the Woods"
...
def test_s3_regression():
    ...
    assert op.skill.name == "Sacred Grove"
