"""Courier S1 "Tactical Formation I" — instant AUTO, refund 8 DP on activation.

Tests cover:
  - S1 config (name, sp_cost=25, initial_sp=10, instant, AUTO, requires_target=False)
  - DP refunded on skill activation (+8 DP)
  - DP gain is instant (no drip after activation)
...
from data.characters.courier import make_courier, _S1_TAG, _S1_DP
...
def test_courier_s1_config():
    op = make_courier(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Tactical Formation I"
...
def test_s1_fires_once_not_drip():
    w = _world()
    op = make_courier(slot="S1")
    op.deployed = True; op.position = (0.0, 1.0); op.atk_cd = 999.0
    w.add_unit(op)
...
def test_s2_regression():
    ...
    assert op.skill.name == "Support Order"
