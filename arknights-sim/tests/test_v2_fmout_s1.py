"""Fmout (Gitano) S1 "Tactical Chant β" — 25s MANUAL, ASPD+75.
...
Tests cover:
  ...
  - S2 slot regression (name="Destiny")
"""
...
from data.characters.fmout import (
    make_fmout, _S1_TAG, _S1_ASPD_BONUS, _S1_ASPD_BUFF_TAG, _S1_DURATION,
)
...
def test_fmout_s1_config():
    op = make_fmout(slot="S1")
    sk = op.skill
...
def test_s2_slot_config():
    op = make_fmout(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Destiny"
