"""Meteo S2 "Shock Zone" — 15s AUTO, ATK+50%, wider splash radius (1.1 → 1.8).

Tests cover:
  - S2 config (name, sp_cost=45, initial_sp=20, duration=15s, behavior_tag)
  - ATK buff: effective_atk = base * (1 + 0.5)
  - splash_radius increased to 1.8 during skill
  - ATK buff cleared on skill end
  - splash_radius reverts to base (1.1) on skill end
...
from data.characters.meteo import (
    make_meteo, _S2_TAG, _S2_ATK_RATIO, _S2_ATK_BUFF_TAG, _S2_SPLASH_RADIUS, _S2_DURATION,
)
...
def test_meteo_s2_config():
    op = make_meteo(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Shock Zone"
...
def test_s2_splash_radius_increased():
    w = _world()
    op = make_meteo(slot="S2")
    base_splash = op.splash_radius  # 1.1
    op.deployed = True; op.position = (0.0, 1.0); op.atk_cd = 999.0
    w.add_unit(op)
    _enemy(w, 1.0, 1.0)
    ...
    assert op.splash_radius == _S2_SPLASH_RADIUS
    assert op.splash_radius > base_splash
...
def test_s2_splash_radius_reverts_on_end():
    w = _world()
    op = make_meteo(slot="S2")
    base_splash = op.splash_radius
    op.deployed = True; op.position = (0.0, 1.0); op.atk_cd = 999.0
    w.add_unit(op)
    _enemy(w, 1.0, 1.0)
...
def test_s3_regression():
    op = make_meteo(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Meteorite Barrage"
