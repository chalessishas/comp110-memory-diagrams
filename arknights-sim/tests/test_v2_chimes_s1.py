"""Chimes S1 "ATK Up γ" — 30s MANUAL, ATK+100%.
...
from data.characters.chimes import (
    make_chimes, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_chimes_s1_config():
    op = make_chimes(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_chimes(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Homeland Melody"
