"""Tests for Beanstalk S3 "Grand Rally" — 20 DP + block→3 + ATK+80%, 25s MANUAL."""
...
from data.characters.beanstalk import make_beanstalk, _S3_DP, _S3_ATK_RATIO, _S3_DURATION, _S3_BUFF_TAG
...
def test_s3_grants_dp():
    ...
    manual_trigger(w, op)
    assert w.global_state.dp == dp_before + _S3_DP


def test_s3_block_raised_to_3():
    w = _world()
    op = make_beanstalk(slot="S3")
    op.deployed = True
    op.position = (0.0, 1.0)
...
def test_s3_block_restored_on_end():
    w = _world()
    op = make_beanstalk(slot="S3")
    op.deployed = True
    op.position = (0.0, 1.0)
