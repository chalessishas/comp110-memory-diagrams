"""Beewax S1 "Sandstorm Spread" — 20s MANUAL, ATK+60%.

Tests cover:
  - S1 config (name, sp_cost=18, initial_sp=8, duration=20s, MANUAL)
...
from data.characters.beewax import (
    make_beewax, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_beewax_s1_config():
    op = make_beewax(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Sandstorm Spread"
...
def test_s2_regression():
    op = make_beewax(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Sandstorm Focus"
