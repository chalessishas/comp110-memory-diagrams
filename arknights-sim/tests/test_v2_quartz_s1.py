"""Quartz S1 "ATK Up β" — 25s MANUAL, ATK+80%.
...
from data.characters.quartz import (
    make_quartz, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_quartz_s1_config():
    op = make_quartz(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_quartz(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "All-Out Clash"
