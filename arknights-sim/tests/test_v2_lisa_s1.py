"""Lisa S1 "All Out" — 30s MANUAL, ATK+80%, ASPD+30.

Tests cover:
  - S1 config (sp_cost=20, initial_sp=0, duration=30s)
...
from data.characters.lisa import (
    make_lisa,
    _S1_TAG, _S1_ATK_RATIO, _S1_ASPD_VALUE,
...
def test_lisa_s1_config():
    op = make_lisa(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.sp_cost == 20 and sk.initial_sp == 0
