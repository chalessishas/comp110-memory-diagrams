"""Robrta S2 "Trade Secrets" — ATK+25% for 20s, AUTO trigger (requires_target).
...
from data.characters.robrta import (
    make_robrta, _S2_ATK_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_robrta_s2_config():
    op = make_robrta(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Trade Secrets"
...
def test_s3_regression():
    ...
    assert op.skill.name == "Grand Merchant"
