"""Pinecone S2 "Heavy Barrage" — ATK+30% for 20s, AUTO trigger (requires_target).
...
from data.characters.pinecone import (
    make_pinecone, _S2_ATK_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_pinecone_s2_config():
    op = make_pinecone(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Heavy Barrage"
...
def test_s3_regression():
    ...
    assert op.skill.name == "Grand Barrage"
