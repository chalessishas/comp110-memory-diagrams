"""Beagle S1 "DEF Up α" — 30s MANUAL, DEF+50%.

Tests cover:
  - S1 config (name, sp_cost=40, initial_sp=0, duration=30s, MANUAL)
...
from data.characters.beagle import (
    make_beagle, _S1_TAG, _S1_DEF_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_beagle_s1_config():
    op = make_beagle(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "DEF Up α"
