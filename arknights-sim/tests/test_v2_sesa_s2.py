"""Sesa S2 "Drumroll" — ATK+80% for 20s, AUTO trigger (requires_target).
...
from data.characters.sesa import (
    make_sesa, _S2_ATK_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_sesa_s2_config():
    op = make_sesa(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Drumroll"
...
def test_s3_regression():
    op = make_sesa(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Percussion Resonance II"
