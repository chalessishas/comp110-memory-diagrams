"""Firewatch S2 "Flash Arrow" — ATK+60% for 15s, AUTO trigger (requires_target).
...
from data.characters.firewatch import (
    make_firewatch, _S2_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_firewatch_s2_config():
    op = make_firewatch(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Flash Arrow"
...
def test_s3_regression():
    op = make_firewatch(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Starburst Arrow"
