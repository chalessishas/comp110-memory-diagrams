"""Lappland S2 "Starvation" — ATK+80% for 30s, AUTO trigger, requires_target.
...
from data.characters.lappland import (
    make_lappland, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_lappland_s2_config():
    op = make_lappland(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Starvation"
...
def test_s3_regression():
    op = make_lappland(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Roaring Flare"
