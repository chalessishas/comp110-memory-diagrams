"""Zima S2 "Battle Cry" — ATK+80% for 25s, AUTO trigger (requires_target).
...
from data.characters.zima import (
    make_zima, _S2_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_zima_s2_config():
    op = make_zima(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Battle Cry"
...
def test_s3_regression():
    op = make_zima(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "March Order"
