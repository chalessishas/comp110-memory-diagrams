"""Lumen S2 "Group Recovery" — ATK+30% for 15s, AUTO trigger.
...
from data.characters.lumen import (
    make_lumen, _S2_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_lumen_s2_config():
    op = make_lumen(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Group Recovery"
...
def test_s3_regression():
    op = make_lumen(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Emergency Protocol"
