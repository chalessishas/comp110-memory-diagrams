...
from data.characters.bluep import (
    make_bluep, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_bluep_s2_config():
    op = make_bluep(slot="S2")
    sk = op.skill
...
def test_s1_slot_config():
    op = make_bluep(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Twinshot - Auto"
