...
from data.characters.kafka import (
    make_kafka, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_kafka_s2_config():
    k = make_kafka(slot="S2")
    sk = k.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Electrocute"
...
def test_s2_atk_buff_applied():
    ...
    assert k.effective_atk > base_atk, "S2 must increase ATK"
    assert any(b.source_tag == _S2_BUFF_TAG for b in k.buffs), "ATK buff must be present"
...
def test_s2_atk_amount():
    ...
    assert k.effective_atk == expected, f"expected ATK={expected}, got {k.effective_atk}"
...
def test_s2_buff_cleared_on_end():
    ...
    assert not any(b.source_tag == _S2_BUFF_TAG for b in k.buffs), "buff must be removed"
    assert k.effective_atk == base_atk, "ATK must return to base after S2"
