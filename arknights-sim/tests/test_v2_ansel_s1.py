"""Ansel S1 "Heal Range Enhancement" — 25s MANUAL, ATK+40%.

Tests cover:
  - S1 config (name, sp_cost=35, initial_sp=10, duration=25s, MANUAL)
  - ATK (heal power) buff applied on trigger
...
from data.characters.ansel import (
    make_ansel, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_ansel_s1_config():
    op = make_ansel(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Heal Range Enhancement"
