"""Caper S2 "Double the Fun" — 20s MANUAL, ATK+60%.

Tests cover:
  - S2 config (name, sp_cost=25, initial_sp=15, duration=20s, MANUAL)
...
from data.characters.caper import (
    make_caper, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_caper_s2_config():
    op = make_caper(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Double the Fun"
