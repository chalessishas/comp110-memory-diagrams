"""Ncdeer S1 "Bringing Good Fortune" — 25s MANUAL, ATK+60%.

Tests cover:
  - S1 config (sp_cost=30, initial_sp=12, duration=25s)
...
from data.characters.ncdeer import (
    make_ncdeer,
...
def test_ncdeer_s1_config():
    op = make_ncdeer(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.sp_cost == 30 and sk.initial_sp == 12
