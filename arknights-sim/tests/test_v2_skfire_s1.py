"""Skfire (Skyfire) S1 "ATK Up γ" — 30s MANUAL, ATK+100%.
...
from data.characters.skfire import (
    make_skfire, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_skfire_s1_config():
    op = make_skfire(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_skfire(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Fire of Heaven"
