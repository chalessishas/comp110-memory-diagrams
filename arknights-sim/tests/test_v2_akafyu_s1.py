"""Akafuyu S1 "Raito no Kamae" — 12s MANUAL, ATK+80%.

Tests cover:
  - S1 config (name, sp_cost=16, initial_sp=0, duration=12s, MANUAL)
...
from data.characters.akafyu import (
    make_akafyu, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_akafyu_s1_config():
    op = make_akafyu(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Raito no Kamae"
...
def test_s2_regression():
    op = make_akafyu(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Jumonji"
