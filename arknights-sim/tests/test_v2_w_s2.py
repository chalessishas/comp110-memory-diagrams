"""W S2 "Extra Supplies" — ATK+60% for 15s, AUTO trigger.
...
from data.characters.w import (
    make_w, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_w_s2_config():
    op = make_w(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Extra Supplies"
...
def test_s2_atk_buff_applied():
    ...
    world.add_unit(op)
    _enemy(world, 1.0, 1.0)

    op.skill.sp = float(op.skill.sp_cost)
    world.tick()

    assert op.effective_atk > base_atk, "S2 must increase ATK"
    assert any(b.source_tag == _S2_BUFF_TAG for b in op.buffs), "ATK buff must be present"
...
def test_s2_atk_amount():
    ...
    assert op.effective_atk == expected, f"expected ATK={expected}, got {op.effective_atk}"
...
def test_s2_buff_cleared_on_end():
    ...
    assert not any(b.source_tag == _S2_BUFF_TAG for b in op.buffs), "buff must be removed"
    assert op.effective_atk == base_atk, "ATK must return to base after S2"
...
def test_s3_regression():
    ...
    assert op.skill.name == "Chaos Edict"
