...
from data.characters.robin import make_robin, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_robin_s1_config():
    op = make_robin(slot="S1")
    sk = op.skill
...
def test_robin_s2_config():
    op = make_robin(slot="S2")
    sk = op.skill
...
def test_robin_s3_config():
    op = make_robin(slot="S3")
    sk = op.skill
