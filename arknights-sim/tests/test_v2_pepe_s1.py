from data.characters.pepe import make_pepe, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_pepe_s1_config():
    op = make_pepe(slot="S1")
    sk = op.skill
...
def test_pepe_s2_config():
    op = make_pepe(slot="S2")
    sk = op.skill
...
def test_pepe_s3_config():
    op = make_pepe(slot="S3")
    sk = op.skill
