"""Frncat (Mousse) S2 "Fury" — 40s MANUAL, ATK+75% DEF+75%.

Tests cover:
  - S2 config (name, sp_cost=80, initial_sp=45, duration=40s, MANUAL)
...
from data.characters.frncat import (
    make_frncat, _S2_TAG, _S2_ATK_RATIO, _S2_DEF_RATIO,
...
def test_frncat_s2_config():
    op = make_frncat(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Fury"
    assert sk.sp_cost == 80
    assert sk.initial_sp == 45
...
def test_s1_regression():
    op = make_frncat(slot="S1")
    assert op.skill is not None and op.skill.slot == "S1"
    assert op.skill.name == "Scratch"
