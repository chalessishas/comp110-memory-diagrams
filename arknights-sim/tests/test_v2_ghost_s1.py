"""Ghost (Specter) S1 "ATK Up γ" — 30s MANUAL, ATK+100%.
...
from data.characters.ghost import (
    make_ghost, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_ghost_s1_config():
    op = make_ghost(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_ghost(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Bone Fracture"
