...
from data.characters.utage import make_utage, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_utage_s1_config():
    op = make_utage(slot="S1")
    sk = op.skill
...
def test_utage_s2_config():
    op = make_utage(slot="S2")
    sk = op.skill
...
def test_utage_s3_config():
    op = make_utage(slot="S3")
    sk = op.skill
