"""Ethan S2 "Venom Burst" — ATK+50% for 15s, AUTO trigger (requires_target).
...
from data.characters.ethan import (
    make_ethan, _S2_BUFF_TAG, _S2_ATK_RATIO, _S2_DURATION, _S2_TAG,
)
...
def test_ethan_s2_config():
    op = make_ethan(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Venom Burst"
...
def test_s3_regression():
    ...
    assert op.skill.name == "Binding Storm"
