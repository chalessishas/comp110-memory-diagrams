"""Hazel (chnut) S1 stub / S2 "Earthburst" — 35s MANUAL, ASPD+100.

Tests cover:
  - S1 config (sp_cost=14, initial_sp=6, duration=6s)
  - S2 config (name, sp_cost=74, initial_sp=30)
...
from data.characters.chnut import (
    make_chnut, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_ASPD_BONUS, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_chnut_s2_config():
    op = make_chnut(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Earthburst"
    assert sk.sp_cost == 74 and sk.initial_sp == 30
