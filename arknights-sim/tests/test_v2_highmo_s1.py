"""Highmore S2 'Evanescence from Paranoia' — 20s MANUAL, ATK+50%."""
...
from data.characters.highmo import (
    make_highmo, _S1_TAG, _S1_DURATION, _S2_TAG,
...
def test_highmo_s1_config():
    op = make_highmo(slot="S1")
    sk = op.skill
...
def test_highmo_s2_config():
    op = make_highmo(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Evanescence from Paranoia"
    assert sk.sp_cost == 34 and sk.initial_sp == 14
