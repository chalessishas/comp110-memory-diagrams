"""Falco (Plume) S1 "Swift Strike α" — 25s MANUAL, ATK+25% ASPD+25.

Tests cover:
  - S1 config (name, sp_cost=45, initial_sp=0, duration=25s, MANUAL)
...
from data.characters.falco import (
    make_falco, _S1_TAG, _S1_ATK_RATIO, _S1_ASPD_BONUS,
...
def test_falco_s1_config():
    op = make_falco(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Swift Strike α"
