from data.characters.lmlee import make_lmlee, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_lmlee_s1_config():
    op = make_lmlee(slot="S1")
    sk = op.skill
...
def test_lmlee_s2_config():
    op = make_lmlee(slot="S2")
    sk = op.skill
...
def test_lmlee_s3_config():
    op = make_lmlee(slot="S3")
    sk = op.skill
