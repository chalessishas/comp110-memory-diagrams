"""Spuria S1 "Hollow Burst" — 8s AUTO, ATK+80% for duration.

Tests cover:
  - S1 config (name, sp_cost=10, initial_sp=5, duration=8s, AUTO, requires_target=False)
  - ATK buff applied: effective_atk = base * (1 + 0.8)
...
from data.characters.spuria import (
    make_spuria, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_spuria_s1_config():
    op = make_spuria(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Hollow Burst"
...
def test_s2_regression():
    op = make_spuria(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Puppet Control"
