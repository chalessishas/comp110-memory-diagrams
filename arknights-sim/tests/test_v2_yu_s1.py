...
from data.characters.yu import make_yu, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION

def test_yu_s1_config():
    op = make_yu(slot="S1")
    sk = op.skill
...
def test_yu_s3_config():
    op = make_yu(slot="S3")
    sk = op.skill
    assert sk is not None and sk.slot == "S3"
    assert sk.sp_cost == 55 and sk.initial_sp == 27
