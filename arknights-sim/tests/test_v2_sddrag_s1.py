"""Sddrag (Reed) S1 "Swift Strike γ" — 35s MANUAL, ATK+45% ASPD+45.
...
from data.characters.sddrag import (
    make_sddrag, _S1_TAG, _S1_ATK_RATIO, _S1_ASPD_BONUS,
...
def test_sddrag_s1_config():
    op = make_sddrag(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_sddrag(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Soul Spark"
