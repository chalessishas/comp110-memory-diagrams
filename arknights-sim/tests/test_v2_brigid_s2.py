"""Brigid S2 "Bring to Heel" — 25s MANUAL, ATK+50%.

Tests cover:
  - S2 config (name, sp_cost=25, initial_sp=15, duration=25s, MANUAL)
...
from data.characters.brigid import (
    make_brigid, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_brigid_s2_config():
    op = make_brigid(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Bring to Heel"
...
def test_s1_slot_config():
    op = make_brigid(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Familiarity"
