from data.characters.hpsts import make_hpsts, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_hpsts_s1_config():
    op = make_hpsts(slot="S1")
    sk = op.skill
...
def test_hpsts_s2_config():
    op = make_hpsts(slot="S2")
    sk = op.skill
...
def test_hpsts_s3_config():
    op = make_hpsts(slot="S3")
    sk = op.skill
