"""Swire S2 'Cooperative Combat' — 21s MANUAL, ATK+50%.

Tests cover:
  - S2 config (sp_cost=50, initial_sp=20, duration=21s, MANUAL)
  - ATK buff applied on trigger
  - ATK buff cleared after skill ends
  - S1 config regression
"""
...
from data.characters.swire import (
    make_swire, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_swire_s2_config():
    op = make_swire(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Cooperative Combat"
