"""Durnar S2 "Iron Defense" — 25s AUTO, DEF+50%, block increases from 3 to 4.

Tests cover:
  - S2 config (name, sp_cost, initial_sp, duration=25s, behavior_tag)
  - DEF buff applied: effective_def = base_def * (1 + 0.5)
  - Block increases to 4 on skill start
  - DEF buff cleared on skill end
  - Block returns to 3 on skill end
  - S3 regression (slot/name)
"""
from __future__ import annotations
import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

from core.world import World
from core.state.tile_state import TileGrid, TileState
from core.state.unit_state import UnitState
from core.types import TileType, TICK_RATE, Faction
from core.systems import register_default_systems
from data.characters.durnar import (
    make_durnar, _S2_TAG, _S2_DEF_RATIO, _S2_DEF_BUFF_TAG, _S2_DURATION,
    _S2_BASE_BLOCK, _S2_BLOCK_BONUS,
)


def _world() -> World:
    grid = TileGrid(width=6, height=3)
    for x in range(6):
        for y in range(3):
            grid.set_tile(TileState(x=x, y=y, type=TileType.GROUND))
    w = World(tile_grid=grid)
    w.global_state.dp_gain_rate = 0.0
    register_default_systems(w)
    return w


def _enemy(world: World, x: float, y: float) -> UnitState:
    e = UnitState(name="Slug", faction=Faction.ENEMY, max_hp=99999, atk=0, defence=0, res=0.0)
    e.alive = True; e.deployed = True; e.position = (x, y)
    world.add_unit(e)
    return e


def test_durnar_s2_config():
    op = make_durnar(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.duration == _S2_DURATION
    assert sk.behavior_tag == _S2_TAG


def test_s2_def_buff_applied():
    w = _world()
    op = make_durnar(slot="S2")
    base_def = op.effective_def
    op.deployed = True; op.position = (0.0, 1.0); op.atk_cd = 999.0
    w.add_unit(op)
    _enemy(w, 0.0, 1.0)  # same tile as Durnar — range_shape ((0,0)) only targets co-located enemies

    op.skill.sp = float(op.skill.sp_cost)
    w.tick()

    expected = int(base_def * (1 + _S2_DEF_RATIO))
    assert op.effective_def == expected
    assert any(b.source_tag == _S2_DEF_BUFF_TAG for b in op.buffs)


def test_s2_block_increases_to_4():
    w = _world()
    op = make_durnar(slot="S2")
    assert op.block == _S2_BASE_BLOCK  # 3 before skill
    op.deployed = True; op.position = (0.0, 1.0); op.atk_cd = 999.0
    w.add_unit(op)
    _enemy(w, 0.0, 1.0)  # same tile as Durnar — range_shape ((0,0)) only targets co-located enemies

    op.skill.sp = float(op.skill.sp_cost)
    w.tick()

    assert op.block == _S2_BASE_BLOCK + _S2_BLOCK_BONUS


def test_s2_def_buff_cleared_on_end():
    w = _world()
    op = make_durnar(slot="S2")
    base_def = op.effective_def
    op.deployed = True; op.position = (0.0, 1.0); op.atk_cd = 999.0
    w.add_unit(op)
    _enemy(w, 0.0, 1.0)  # same tile as Durnar — range_shape ((0,0)) only targets co-located enemies

    op.skill.sp = float(op.skill.sp_cost)
    for _ in range(int(TICK_RATE * (_S2_DURATION + 1.0))):
        w.tick()

    assert not any(b.source_tag == _S2_DEF_BUFF_TAG for b in op.buffs)
    assert op.effective_def == base_def


def test_s2_block_restored_on_end():
    w = _world()
    op = make_durnar(slot="S2")
    op.deployed = True; op.position = (0.0, 1.0); op.atk_cd = 999.0
    w.add_unit(op)
    _enemy(w, 0.0, 1.0)  # same tile as Durnar — range_shape ((0,0)) only targets co-located enemies

    op.skill.sp = float(op.skill.sp_cost)
    for _ in range(int(TICK_RATE * (_S2_DURATION + 1.0))):
        w.tick()

    assert op.block == _S2_BASE_BLOCK


def test_s3_regression():
    op = make_durnar(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Iron Fortress"
