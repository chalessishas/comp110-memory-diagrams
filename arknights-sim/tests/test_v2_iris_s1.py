from data.characters.iris import make_iris, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_iris_s1_config():
    op = make_iris(slot="S1")
    sk = op.skill
...
def test_iris_s2_config():
    op = make_iris(slot="S2")
    sk = op.skill
...
def test_iris_s3_config():
    op = make_iris(slot="S3")
    sk = op.skill
