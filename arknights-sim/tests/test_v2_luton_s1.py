from data.characters.luton import make_luton, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_luton_s1_config():
    op = make_luton(slot="S1")
    sk = op.skill
...
def test_luton_s2_config():
    op = make_luton(slot="S2")
    sk = op.skill
...
def test_luton_s3_config():
    op = make_luton(slot="S3")
    sk = op.skill
