"""Leto S1 "Swift Strike γ" — 35s MANUAL, ATK+45% ASPD+45.
...
from data.characters.leto import (
    make_leto, _S1_TAG, _S1_ATK_RATIO, _S1_ASPD_BONUS,
...
def test_leto_s1_config():
    op = make_leto(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_leto(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Answer the Call"
