"""Vendla S2 "Tactical Chant" — 30s MANUAL, ASPD+90.

Tests cover:
  - S1 config (stub: sp_cost=10, initial_sp=5)
...
from data.characters.vendla import (
    make_vendla,
...
def test_vendla_s1_config():
    op = make_vendla(slot="S1")
    sk = op.skill
...
def test_vendla_s2_config():
    op = make_vendla(slot="S2")
    sk = op.skill
