"""Czerny S1 "Flying Fingers" — 35s MANUAL, ATK+80%, RES+100%.

Tests cover:
  - S1 config (name, sp_cost=35, initial_sp=20, duration=35s)
  - S1 ATK buff applied on trigger
  - S1 RES buff applied on trigger
...
from data.characters.pianst import (
    make_pianst,
    _S1_TAG, _S1_ATK_RATIO, _S1_RES_RATIO,
    _S1_ATK_BUFF_TAG, _S1_RES_BUFF_TAG, _S1_DURATION,
    _S2_TAG,
)
...
def test_pianst_s1_config():
    op = make_pianst(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Flying Fingers"
    assert sk.sp_cost == 35 and sk.initial_sp == 20
...
def test_s1_res_buff_applied():
    ...
    assert any(b.source_tag == _S1_RES_BUFF_TAG and b.value == _S1_RES_RATIO
