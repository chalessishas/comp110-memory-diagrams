"""Lava the Purgatory S2 "Scorched Earth" — splash 1.5, ATK+30%, 25s AUTO.

Tests cover:
  - S2 config (slot, name, sp_cost, initial_sp, duration, behavior_tag)
  - splash_radius expands to 1.5 on skill start
  - ATK buff applied on start: effective_atk = base * (1 + 0.30)
  - Both restored to base values after skill ends
...
from data.characters.lava2 import (
    make_lava2, _S2_TAG, _S2_ATK_RATIO, _S2_ATK_BUFF_TAG,
    _S2_SPLASH_RADIUS, _S2_DURATION, _TRAIT_SPLASH_RADIUS,
)
...
def test_lava2_s2_config():
    op = make_lava2(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Scorched Earth"
...
def test_s2_splash_radius_expanded():
    w = _world()
    op = make_lava2(slot="S2")
...
def test_s2_restored_on_end():
    ...
    assert op.splash_radius == _TRAIT_SPLASH_RADIUS


def test_s3_regression():
    op = make_lava2(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Volcanic Surge"
