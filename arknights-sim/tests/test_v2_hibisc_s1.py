"""Hibiscus S1 "Healing Enhancement α" — 20s MANUAL, ATK+50% (heal boost).

Tests cover:
  - S1 config (sp_cost=30, initial_sp=0, duration=20s)
...
from data.characters.hibisc import (
    make_hibisc,
...
def test_hibisc_s1_config():
    op = make_hibisc(slot="S1")
    sk = op.skill
    assert sk is not None and sk.slot == "S1"
    assert sk.name == "Healing Enhancement α"
