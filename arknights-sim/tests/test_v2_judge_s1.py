from data.characters.judge import make_judge, _S1_TAG, _S1_DURATION, _S2_TAG, _S2_DURATION, _S3_TAG, _S3_DURATION


def test_judge_s1_config():
    op = make_judge(slot="S1")
    sk = op.skill
...
def test_judge_s2_config():
    op = make_judge(slot="S2")
    sk = op.skill
...
def test_judge_s3_config():
    op = make_judge(slot="S3")
    sk = op.skill
