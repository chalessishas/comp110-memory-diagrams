"""Swllow S2 "Backflow" — 20s MANUAL, ATK+40%.

Tests cover:
  - S1 config (stub: sp_cost=5, initial_sp=0)
  - S2 config (sp_cost=30, initial_sp=15, duration=20s)
...
from data.characters.swllow import (
    make_swllow,
...
def test_swllow_s1_config():
    op = make_swllow(slot="S1")
    sk = op.skill
...
def test_swllow_s2_config():
    op = make_swllow(slot="S2")
    sk = op.skill
