"""Archetto S2 "Tactical Positioning" — ATK+80% for 15s, AUTO_TIME trigger.
...
from data.characters.archetto import (
    make_archetto, _S2_TAG, _S2_ATK_RATIO, _S2_BUFF_TAG, _S2_DURATION,
)
...
def test_archetto_s2_config():
    op = make_archetto(slot="S2")
    sk = op.skill
    assert sk is not None and sk.slot == "S2"
    assert sk.name == "Tactical Positioning"
...
def test_s3_regression():
    op = make_archetto(slot="S3")
    assert op.skill is not None and op.skill.slot == "S3"
    assert op.skill.name == "Tempest"
