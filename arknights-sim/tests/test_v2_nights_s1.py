"""Nights (Haze) S1 "ATK Up β" — 25s MANUAL, ATK+80%.
...
from data.characters.nights import (
    make_nights, _S1_TAG, _S1_ATK_RATIO, _S1_BUFF_TAG, _S1_DURATION,
)
...
def test_nights_s1_config():
    op = make_nights(slot="S1")
    sk = op.skill
...
def test_s2_regression():
    op = make_nights(slot="S2")
    assert op.skill is not None and op.skill.slot == "S2"
    assert op.skill.name == "Crimson Eyes"
