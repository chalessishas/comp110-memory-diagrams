"""Susuro S2 "Healing Enhancement β" — 25s MANUAL, ATK+70%.

Tests cover:
  - S1 config (stub: sp_cost=8, initial_sp=0)
  - S2 config (sp_cost=30, initial_sp=10, duration=25s)
...
from data.characters.susuro import (
    make_susuro,
...
def test_susuro_s1_config():
    op = make_susuro(slot="S1")
    sk = op.skill
...
def test_susuro_s2_config():
    op = make_susuro(slot="S2")
    sk = op.skill
