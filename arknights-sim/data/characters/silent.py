...
_S1_TAG = "silent_s1"; _S1_DURATION = 0.0
_S2_TAG = "silent_s2"; _S2_DURATION = 20.0

def make_silent(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Silent"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Silent S1", slot="S1", sp_cost=8, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.AUTO, requires_target=False, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Silent S2", slot="S2", sp_cost=30, initial_sp=15,
