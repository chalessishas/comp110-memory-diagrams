"""Haak (char) — 5★ Specialist (Executor).
...
_S1_TAG = "haak_s1"; _S1_DURATION = 0.0
_S2_TAG = "haak_s2"; _S2_DURATION = 25.0

def make_haak(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Haak"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Haak S1", slot="S1", sp_cost=5, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Haak S2", slot="S2", sp_cost=40, initial_sp=20,
