...
def make_folivo(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Folinic"
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Folinic S1", slot="S1", sp_cost=20, initial_sp=10,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Folinic S2", slot="S2", sp_cost=45, initial_sp=22,
