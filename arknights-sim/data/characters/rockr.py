...
from data.characters.generated.rockr import make_rockr as _base_stats

CAST_RANGE = RangeShape(tiles=tuple((dx, dy) for dx in range(0, 3) for dy in range(-1, 2)))
_S1_TAG = "rockr_s1"; _S1_DURATION = 0.0
_S2_TAG = "rockr_s2"; _S2_DURATION = 15.0

def make_rockr(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Rockr"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Rockr S1", slot="S1", sp_cost=10, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.AUTO, requires_target=False, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Rockr S2", slot="S2", sp_cost=30, initial_sp=15,
