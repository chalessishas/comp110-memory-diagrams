"""Hellagur (赫拉格) — 6★ Guard (Dreadnought).
...
_S1_TAG = "helage_s1"; _S1_DURATION = 0.0
_S2_TAG = "helage_s2"; _S2_DURATION = 30.0
_S3_TAG = "helage_s3"; _S3_DURATION = 35.0

def make_helage(slot: str = "S3") -> UnitState:
    op = _base_stats()
    op.name = "Hellagur"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Hellagur S1", slot="S1", sp_cost=3, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Hellagur S2", slot="S2", sp_cost=50, initial_sp=25,
            duration=_S2_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S2_TAG)
    elif slot == "S3":
        op.skill = SkillComponent(name="Hellagur S3", slot="S3", sp_cost=65, initial_sp=32,
