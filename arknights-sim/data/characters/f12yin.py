"""F12yin (char) — 6★ Guard (Swordmaster).
...
_S1_TAG = "f12yin_s1"; _S1_DURATION = 0.0
_S2_TAG = "f12yin_s2"; _S2_DURATION = 25.0
_S3_TAG = "f12yin_s3"; _S3_DURATION = 30.0

def make_f12yin(slot: str = "S3") -> UnitState:
    op = _base_stats()
    op.name = "F12yin"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="F12yin S1", slot="S1", sp_cost=3, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="F12yin S2", slot="S2", sp_cost=45, initial_sp=22,
            duration=_S2_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S2_TAG)
    elif slot == "S3":
        op.skill = SkillComponent(name="F12yin S3", slot="S3", sp_cost=60, initial_sp=30,
