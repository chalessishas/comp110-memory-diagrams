...
from data.characters.generated.ela import make_ela as _base_stats
...
def make_ela(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Ela"
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Ela S1", slot="S1", sp_cost=4, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Ela S2", slot="S2", sp_cost=40, initial_sp=20,
