...
from data.characters.generated.vodfox import make_vodfox as _base_stats

SUP_RANGE = RangeShape(tiles=tuple((dx, dy) for dx in range(0, 3) for dy in range(-1, 2)))
_S1_TAG = "vodfox_s1"; _S1_DURATION = 0.0
_S2_TAG = "vodfox_s2"; _S2_DURATION = 10.0

def make_vodfox(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Vodfox"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Vodfox S1", slot="S1", sp_cost=15, initial_sp=8,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.AUTO, requires_target=False, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Vodfox S2", slot="S2", sp_cost=25, initial_sp=12,
