"""Almond (杏仁) — Specialist (char_4105).
...
from data.characters.generated.almond import make_almond as _base_stats
...
def make_almond(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Almond"
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Almond S1", slot="S1", sp_cost=4, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Almond S2", slot="S2", sp_cost=45, initial_sp=20,
