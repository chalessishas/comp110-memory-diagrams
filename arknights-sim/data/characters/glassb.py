...
from data.characters.generated.glassb import make_glassb as _base_stats
...
def make_glassb(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Glassb"
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Glassb S1", slot="S1", sp_cost=20, initial_sp=10,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Glassb S2", slot="S2", sp_cost=40, initial_sp=20,
