...
from data.characters.generated.tecno import make_tecno as _base_stats

CAST_RANGE = RangeShape(tiles=tuple((dx, dy) for dx in range(0, 3) for dy in range(-1, 2)))
_S1_TAG = "tecno_s1"; _S1_DURATION = 0.0
_S2_TAG = "tecno_s2"; _S2_DURATION = 15.0

def make_tecno(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Tecno"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Tecno S1", slot="S1", sp_cost=10, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.AUTO, requires_target=False, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Tecno S2", slot="S2", sp_cost=30, initial_sp=15,
