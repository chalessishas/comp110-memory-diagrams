"""Ghost Shark Alter (归溟幽灵鲨) — 6★ Specialist (char_1023).
...
def make_ghost2(slot: str = "S3") -> UnitState:
    op = _base_stats()
    op.name = "Ghost Shark Alter"
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Ghost Shark Alter S1", slot="S1", sp_cost=5, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Ghost Shark Alter S2", slot="S2", sp_cost=50, initial_sp=25,
            duration=_S2_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S2_TAG,
        )
    elif slot == "S3":
        op.skill = SkillComponent(
            name="Ghost Shark Alter S3", slot="S3", sp_cost=70, initial_sp=35,
