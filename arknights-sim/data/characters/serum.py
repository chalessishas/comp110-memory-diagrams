...
from data.characters.generated.serum import make_serum as _base_stats

CAST_RANGE = RangeShape(tiles=tuple((dx, dy) for dx in range(0, 3) for dy in range(-1, 2)))
_S1_TAG = "serum_s1"; _S1_DURATION = 0.0
_S2_TAG = "serum_s2"; _S2_DURATION = 15.0

def make_serum(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Serum"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Serum S1", slot="S1", sp_cost=10, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.AUTO, requires_target=False, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Serum S2", slot="S2", sp_cost=35, initial_sp=18,
