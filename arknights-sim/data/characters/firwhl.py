"""Firepeak (火哨) — 5★ Defender (Sentinel).

S1: sp_cost=5, initial_sp=0, instant, AUTO_DEFENSIVE, AUTO (stub).
...
def make_firwhl(slot: str = "S2") -> UnitState:
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Firepeak S1", slot="S1", sp_cost=5, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_DEFENSIVE,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Firepeak S2", slot="S2", sp_cost=55, initial_sp=30,
