"""Ascalon (阿斯卡纶) — Specialist (char_4132, Pusher, block=0).
...
from data.characters.generated.ascln import make_ascln as _base_stats
...
def make_ascln(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Ascalon"
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Ascalon S1", slot="S1", sp_cost=4, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Ascalon S2", slot="S2", sp_cost=40, initial_sp=20,
