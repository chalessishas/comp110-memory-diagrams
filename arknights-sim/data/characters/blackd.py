"""Blackd (char) — 5★ Vanguard (Pioneer).
...
_S1_TAG = "blackd_s1"; _S1_DURATION = 15.0
_S2_TAG = "blackd_s2"; _S2_DURATION = 20.0

def make_blackd(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Blackd"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Blackd S1", slot="S1", sp_cost=20, initial_sp=10,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Blackd S2", slot="S2", sp_cost=40, initial_sp=20,
