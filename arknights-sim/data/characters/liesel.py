...
from data.characters.generated.liesel import make_liesel as _base_stats

CAST_RANGE = RangeShape(tiles=tuple((dx, dy) for dx in range(0, 3) for dy in range(-1, 2)))
_S1_TAG = "liesel_s1"; _S1_DURATION = 15.0
_S2_TAG = "liesel_s2"; _S2_DURATION = 20.0
_S3_TAG = "liesel_s3"; _S3_DURATION = 30.0

def make_liesel(slot: str = "S3") -> UnitState:
    op = _base_stats()
    op.name = "Liesel"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Liesel S1", slot="S1", sp_cost=20, initial_sp=10,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Liesel S2", slot="S2", sp_cost=50, initial_sp=25,
            duration=_S2_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S2_TAG)
    elif slot == "S3":
        op.skill = SkillComponent(name="Liesel S3", slot="S3", sp_cost=70, initial_sp=35,
