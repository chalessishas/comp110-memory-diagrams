"""Regicide (弑君者) — 6★ Specialist (char_1502).
...
from data.characters.generated.crosly import make_crosly as _base_stats
...
def make_crosly(slot: str = "S3") -> UnitState:
    op = _base_stats()
    op.name = "Regicide"
    ...
    if slot == "S1":
        op.skill = SkillComponent(
            name="Regicide S1", slot="S1", sp_cost=4, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG,
        )
    elif slot == "S2":
        op.skill = SkillComponent(
            name="Regicide S2", slot="S2", sp_cost=45, initial_sp=20,
            duration=_S2_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S2_TAG,
        )
    elif slot == "S3":
        op.skill = SkillComponent(
            name="Regicide S3", slot="S3", sp_cost=65, initial_sp=32,
