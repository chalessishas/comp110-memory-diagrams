...
from data.characters.generated.ray import make_ray as _base_stats

SNP_RANGE = RangeShape(tiles=tuple((dx, dy) for dx in range(0, 4) for dy in range(-1, 2)))
_S1_TAG = "ray_s1"; _S1_DURATION = 0.0
_S2_TAG = "ray_s2"; _S2_DURATION = 20.0
_S3_TAG = "ray_s3"; _S3_DURATION = 20.0

def make_ray(slot: str = "S3") -> UnitState:
    op = _base_stats()
    op.name = "Ray"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Ray S1", slot="S1", sp_cost=5, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Ray S2", slot="S2", sp_cost=45, initial_sp=22,
            duration=_S2_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S2_TAG)
    elif slot == "S3":
        op.skill = SkillComponent(name="Ray S3", slot="S3", sp_cost=65, initial_sp=32,
