"""Skybx (Skyfire alter) — 6★ Sniper (Deadeye).
...
S3: sp_cost=70, initial_sp=35, duration=25s, AUTO_TIME, MANUAL (stub).
"""
...
def make_skybx(slot: str = "S3") -> UnitState:
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Skybx S1", slot="S1", sp_cost=5, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Skybx S2", slot="S2", sp_cost=50, initial_sp=25,
