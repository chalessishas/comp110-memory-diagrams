"""Shwaz (Schwarz 史尔特尔) — 6★ Sniper (Deadeye).
...
_S2_TAG = "shwaz_s2"; _S2_DURATION = 25.0
_S3_TAG = "shwaz_s3"; _S3_DURATION = 30.0

def make_shwaz(slot: str = "S3") -> UnitState:
    op = _base_stats()
    op.name = "Schwarz"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Schwarz S1", slot="S1", sp_cost=5, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Schwarz S2", slot="S2", sp_cost=45, initial_sp=22,
            duration=_S2_DURATION, sp_gain_mode=SPGainMode.AUTO_TIME,
            trigger=SkillTrigger.MANUAL, requires_target=False, behavior_tag=_S2_TAG)
    elif slot == "S3":
        op.skill = SkillComponent(name="Schwarz S3", slot="S3", sp_cost=65, initial_sp=32,
