"""Melanite (char) — 5★ Sniper (Deadeye).
...
_S1_TAG = "melnte_s1"; _S1_DURATION = 0.0
_S2_TAG = "melnte_s2"; _S2_DURATION = 20.0

def make_melnte(slot: str = "S2") -> UnitState:
    op = _base_stats()
    op.name = "Melanite"
    ...
    if slot == "S1":
        op.skill = SkillComponent(name="Melanite S1", slot="S1", sp_cost=3, initial_sp=0,
            duration=_S1_DURATION, sp_gain_mode=SPGainMode.AUTO_ATTACK,
            trigger=SkillTrigger.AUTO, requires_target=True, behavior_tag=_S1_TAG)
    elif slot == "S2":
        op.skill = SkillComponent(name="Melanite S2", slot="S2", sp_cost=40, initial_sp=20,
