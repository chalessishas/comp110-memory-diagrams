...
### 34. element_mastery 查询必须过滤 element_name='_overall'
CourseHub 的 `element_mastery` 表每个 (user_id, concept_id) 可能有多行，每行对应不同的 element_name（如 `_overall`, `_lesson_xxx`, 等）。所有显示掌握度的查询**必须加 `.eq("element_name", "_overall")`**，否则 `Map(rows.map(m => [m.concept_id, m]))` 会被非 overall 行覆盖，显示错误掌握度。

profile/page.tsx 已正确过滤，但 dashboard、tree、progress、course overview 均遗漏，导致 mastery bar 静默返回 0 或错误级别。

**代价**：dashboard mastery progress bar 上线后完全无数据，修复用了 2 个 turn。
