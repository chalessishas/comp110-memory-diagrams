...
└──────────┬──────────────┘
           ↓
┌─────────────────────────┐
│  3. Board Renderer       │  Revideo (Revideo fork, headless API)
│     指令 → 板书动画帧    │  按 TTS 时长对齐每段动画
└──────────┬──────────────┘
           ↓
┌─────────────────────────┐
│  4. Compositor           │  Revideo renderVideo() + FFmpeg
│     音频 + 视频 → MP4    │
└─────────────────────────┘
```
...
type BoardAction =
  ...
  | { type: "erase"; target: "all" | string }                 // "all" = 翻页清空；string = 擦除指定元素的 id
...
- 全自动要求零失败率。LLM 填 JSON schema 几乎不会出错（结构化输出），直接写 Revideo 代码有 ~30% 失败率
- 解耦：换渲染引擎不影响 LLM 层，换 LLM 不影响渲染层
- 扩展新操作只改两处：schema 加一个 type + Revideo 加一个模板组件

**为什么 Revideo 而非 Revideo：**
- Revideo 无 headless CLI（Issue #415 至今未关闭），无法在自动管线中运行
- Revideo 是 Revideo 的 fork（3.7k stars），提供 `renderVideo()` Node.js API
- 支持并行渲染（70x 加速）+ 内置音频混合
- API 与 Revideo 同源，生态兼容
...
2. 获得每段音频文件 + 精确时长（毫秒）
3. 将时长传给 Revideo 渲染器，每段板书动画的总时长 = 该段语音时长
4. 板书动作按内容量加权分配时间（write_text: text.length, show_formula: latex.length × 1.5, draw_arrow: 10, highlight: 5, erase: 3）。pause 直接用 duration 值，不参与分配
...
# 带参考教材（LLM 基于教材内容生成脚本，更准确）
npx course-video generate "极限的定义" --source ./calculus-ch2.txt
```
