...
在人类 vs 人类对战之外，引入 **Arena AI** 作为常驻竞技选手：

- **AI 作为对手**: 用户随时可以向 AI 发起挑战，AI 根据知识点"答题"（实际是用 LLM 生成答案）
- **难度分级**: AI 有多个"段位"（Bronze/Silver/Gold/Diamond），低段位 AI 故意答错部分题，高段位全力作答
- **能力标尺**: "击败 Gold AI" 成为用户实力的直观标签，比纯 ELO 更有社交传播力
- **数据飞轮**: AI 答错的题 = 高质量难题，可以反哺题库质量评估

这个机制同时解决冷启动（永远有对手）和用户动机（打败 AI 的成就感）两个问题。Phase 2+ 实现。
